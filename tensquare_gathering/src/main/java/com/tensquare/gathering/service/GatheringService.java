...
public class GatheringService {
	 ...
	 * 	value指的是当前方法的返回值
	 * 	key指的是获取时的唯一标识。
	 *		上面的key和value指的是此方法中的：redistTemplate.opsForValue().set("article_"+id,article);
	 *
	 *  而Cacheable注解上的key和value属性都是用于创建存入唯一标识用的。
	 *  		value：是唯一标识中固定的部分
	 *  		key：是唯一标识中的变量的部分
	 */
	@Cacheable(key="#id",value="article_")
	 ...
	 * @param gathering
	 */
	@CacheEvict(key="#gathering.id",value="article_")
